...
function Install() {
  ...
  const installed = appdata.filter((app) => stored.includes(app.id));
  const removeHendler=(id)=>{
    RemoveAppsDB(id)
    toast("Removed , Refresh the page")
  }
  return (
    <div>
      ...
      <ToastContainer></ToastContainer>
        ...
        {installed.map((app) => (
          <div key={app.id} className="grid grid-cols-1 gap-5 py-2">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white shadow-sm transition-all duration-200">
              ...
              <button onClick={()=>{removeHendler(app.id)} } className="btn btn-success text-white">Uninstall</button>
            </div>
          </div>
        ))}
        </div>
